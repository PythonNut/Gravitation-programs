...
def one_step(initial_position, initial_velocity, k, m, n, step_size):
	force = central_force(k,m,initial_position,n)
	new_position = euler_step(initial_position, initial_velocity, step_size)
	new_velocity = euler_step(initial_velocity, force, step_size)
...
def simulate(initial_position, initial_velocity, k, m, n, step_size, num_steps):
	start_time = time.time()
	steps = 0
	position_list = []
	while steps < num_steps: 
		new_stuff = one_step(initial_position, initial_velocity, k, m, n, step_size)
		initial_position = new_stuff[0]
		initial_velocity = new_stuff[1]
		current_time = step_size*steps 
		position_list.append((np.asarray(initial_position)))
		steps += 1 
		# print(new_stuff)
	stop_time = time.time()
