...
def one_step(position, velocity, k, m, n_tuple, step_size):
	force1 = central_force(k,m,position,n_tuple[0])
	force2 = central_force(k,m,position,n_tuple[1])
	for i in range(len(force2)):
		force2[i] = .1*force2[i]
	force = [0,0,0]
	for i in range(len(force1)):
		force[i] = force1[i] + .1*force2[i]
...
def simulate(position, velocity, k, m, n_tuple, step_size, num_steps):
	start_time = time.time()
	steps = 0
	position_list = []
	while steps < num_steps: 
		new_stuff = one_step(position, velocity, k, m, n_tuple, step_size)
