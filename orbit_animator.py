...
def animate(i):
    steps = 200000                                              # number of timesteps per frame                                                         
...
def wrapper(r_exp_list, position, velocity, k, m, step_size, numsteps):
    ...
    for n in r_exp_list:                                      # iterate through a list of exponent values for r in newton's law of gravitation
        ...
        x_t = np.asarray([backend.simulate(position,velocity,k,m,n,step_size,numsteps)]) # if you're outputting from your own function, make sure that it's an array of position vectors (also arrays!)
